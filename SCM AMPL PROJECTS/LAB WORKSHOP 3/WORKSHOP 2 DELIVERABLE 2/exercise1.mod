...
param t {I,J} >=0 ;#transportation cost per unit from i to j
param transcap {I} >= 0 ; #outbound transportation capacity from facility i
...
#objective function
minimize z: sum {i in I} f[i]*y[i] 
			+ sum{i in I, j in J} (c[i,j]+t[i,j])*x[i,j] 
			+ sum{i in I} h[i]*s[i];
...
s.t. transportation_cap{i in I}: sum{j in J}x[i,j] <= transcap[i]*y[i]