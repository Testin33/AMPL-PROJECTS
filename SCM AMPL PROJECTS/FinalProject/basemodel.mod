...
param X{V} >= 0;
param Y{V} >= 0;
#distance cost - calculated by euclidean distance
param Cdist{i in V, j in V} >= 0 :=
    (if i = j then 0
     else sqrt( (X[i] - X[j])^2 + (Y[i] - Y[j])^2 ));
     
#Travel Time calculated by the distance
#Tiempo de viaje = distancia (modelo base)
param Ttime{i in V, j in V} >= 0 := Cdist[i,j];
...
s.t. Link_SC_1{i in R, j in C, k in K}:
    sum{v in V} x[k,i,v]- sum{v in V} x[k,v,j] <= Mbig * (1 - y[i,j]);

s.t. Link_SC_2{i in R, j in C, k in K}:
    sum{v in V} x[k,v,j] - sum{v in V} x[k,i,v] <= Mbig * (1 - y[i,j]);
# --------------------
...
s.t. Depot_Time{k in K}: s[k,0] = 0;

s.t. Time_Arc{k in K, i in V, j in N: i != j}:
    s[k,j] >= s[k,i] + Ttime[i,j] + O[i] - Mbig * (1 - x[k,i,j]);

s.t. Precedence_SC{i in R, j in C, k in K}:
    s[k,j] >= s[k,i] + Ttime[i,j] + O[i] - Mbig * (1 - y[i,j]);
...
s.t. Max_Route_Length{k in K, i in N}:
    s[k,i] + O[i] + Ttime[i,0] <= L + Mbig * (1 - sum{j in V: j != i} x[k,i,j]);
